...
async def create_feature(data: FeatureCreate, db: AsyncSession = Depends(get_db)):
    ...
    feature = SpatialFeature(name=data.name, geometry=geom, category=data.category, address=data.address)
    db.add(feature)
    await db.commit()
...
async def list_features(db: AsyncSession = Depends(get_db)):
    ...
    rows = result.all()
    return [{"id": r.id, "name": r.name, "category": r.category, "address": r.address, "geometry": json.loads(r.geometry)}for r in rows]
